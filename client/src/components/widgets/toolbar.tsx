...
export default function Toolbar(
    ...
    return (
        <View className='relative w-full mt-auto z-20 left-0 right-0 items-center'>
            ...
            {/* The toolbar itself */}
            <View className='w-full flex flex-row border-2 bg-slate-200 border-slate-200 p-1 gap-6 justify-center'>
                <Button
                variant={"default"}
                disabled={popupOpen}
                className={`bg-[#00000080] transition-opacity duration-200 ${ popupOpen ? 'opacity-0' : 'opacity-100' }`}>
